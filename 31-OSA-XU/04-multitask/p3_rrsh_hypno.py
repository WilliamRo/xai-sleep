...
if IN_LINUX:
  assert len(sys.argv) >= 2, 'Please specify target_id'
  TARGET_ID = int(sys.argv[1])
  PP = int(sys.argv[2]) if len(sys.argv) == 3 else 3000
else:
  TARGET_ID = 7
  PP = 3000
...
# (*)
assert TARGET_ID in (2, 4, 5, 6, 7, 11)
TARGET = [
  'AHI',  # 0
  'age',  # 1
...
PI_FN = f'{OMIX_PREFIX}_{TARGET}_M{M}N{N}_k({k_str})_t({t_str})_PP{PP}{n_str}.pi'
