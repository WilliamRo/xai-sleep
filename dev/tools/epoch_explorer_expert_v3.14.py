...
class EpochExplorer(Pictor):
  ...
  class Keys(Pictor.Keys):
    ...
    STAGE_EPOCH_PKG_DICT = 'stage_epoch_pkg_dict'
  ...
  def selected_package(self):
    ...
    c = self.get_element(self.Keys.CHANNELS)
    epoch = self.get_element(self.Keys.EPOCHS)
    se = self.get_sg_stage_epoch_pkg_dict(self.selected_signal_group)
    return se[self.selected_stage][epoch][:, c]
  ...
  @classmethod
  def get_sg_stage_epoch_pkg_dict(cls, sg: SignalGroup):
    def _init_dict():
      # TODO: expert1
      se_dict = {}
    ...
    return sg.get_from_pocket(
      cls.Keys.STAGE_EPOCH_PKG_DICT, initializer=_init_dict)
...
class RhythmPlotter(Plotter):
  ...
  def _plot_spectrum(self, ax: plt.Axes):
    ...
    # (1) Fetch data to plot
    # sg = self.explorer.selected_signal_group
    stft, a, h = self.explorer.selected_package
