...
sg_file_list, N = freud.generate_clouds(
  SG_PATH, pattern=SG_PATTERN, channels=CHANNELS,
  time_resolutions=TIME_RESOLUTIONS, overwrite=OVERWRITE,
  extractor_dict=extractor_dict, parallel_channel=IN_LINUX,
  return_sg_file_list=True)

n = len(sg_file_list)

console.show_status(f'Progress for probe_config `{PROBE_CONFIG}`: {N - n}/{N}.')
